...
<div class="md:mx-10 py-10 px-4 sm:px-6 lg:px-8">
    ...
    <div class="bg-white shadow-md rounded-lg p-6 mb-8">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight mb-8">Todolist</h2>
    ...
    <div class="space-y-4">
        ...
        <div
            ...
            <div class="flex items-start gap-4 w-full">
                ...
                <div class="flex-1">
                    ...
                    @if ($todo->description)
                    <p class="text-sm text-gray-600 lowercase {{ $todo->is_done ? 'line-through' : '' }}">
                        {{ $todo->description }}
                    </p>
                    @endif
                    <p class="text-xs text-gray-400 mt-1">Created: {{ $todo->created_at->format('d M Y') }}</p>
                </div>
            </div>

            <div class="flex gap-2 ml-4">
                <a href="{{ route('todos.edit', $todo) }}"
                    class="inline-flex items-center justify-center bg-yellow-500 hover:bg-yellow-600 text-white text-sm font-medium px-3 py-1.5 rounded" title="Edit Todo">
                    ✏️
                </a>
                <form action="{{ route('todos.destroy', $todo) }}" method="POST"
                    ...
                    @method('DELETE')
                    <button
                        class="inline-flex items-center justify-center bg-red-600 hover:bg-red-700 text-white text-sm font-medium px-3 py-1.5 rounded" title="Delete Todo">
                        🗑️
                    </button>
                </form>
            </div>
        ...
        @empty
        <p class="text-gray-500 text-center">Todo not found. Add todo soon!</p>
        @endforelse
    </div>
</div>
@endsection